...
// Storage abstraction (Supabase cloud table + localStorage fallback)
const _writeQueues = {};
...
// ===== 3. ENGINE CALCULATIONS & FORMULAS =====
...
const SELLING_METHOD_OPTIONS = [
  "eBay", "eBay Live", "Whatnot", "Whatnot (AU promo hours)",
  "DCSports87", "Fanatics Collect (PWCC)", "ShipMyCards Marketplace",
  "Facebook / local", "Other"
];
...
function estimateSellingFee(method, price) {
  ...
  switch (method) {
    ...
    case "DCSports87": return dcsports87Fee(p);
...
function suggestedSellingMethod(card, listing) {
  ...
  return {
    ...
    why: "eBay Live and Whatnot's cheaper fees only kick in when you actually go live, which needs an approved seller account plus enough volume or a following to fill a stream — not worth building for a single card. Standard eBay works without any of that.",
  };
}
...
function App() {
  ...
  useEffect(() => { if (dataLoaded) storageSet(BUY_STORAGE_KEY, buyList); }, [buyList, dataLoaded]);
  useEffect(() => { if (dataLoaded) storageSet(BOX_STORAGE_KEY, boxBreaks); }, [boxBreaks, dataLoaded]);
  useEffect(() => { if (dataLoaded) storageSet(TARGETS_STORAGE_KEY, targets); }, [targets, dataLoaded]);
  useEffect(() => { if (dataLoaded) storageSet(CONTENT_STORAGE_KEY, contentPlan); }, [contentPlan, dataLoaded]);
  useEffect(() => { if (dataLoaded) storageSet(CONTENT_GOAL_STORAGE_KEY, contentGoal); }, [contentGoal, dataLoaded]);
  useEffect(() => { if (dataLoaded) storageSet(MANUAL_EXPENSES_STORAGE_KEY, manualExpenses); }, [manualExpenses, dataLoaded]);
  useEffect(() => { if (dataLoaded) storageSet(SAVED_SCANS_STORAGE_KEY, savedScans); }, [savedScans, dataLoaded]);
  ...
  function exportAllData() {
    const bundle = {
      ...
      version: 1,
      cards, pokemonCards, buyList, boxBreaks, targets, contentPlan, contentGoal, manualExpenses, savedScans,
    };
    try {
  ...
  function importAllData(file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!data || typeof data !== "object" || data.app !== "CardFlip EV") throw new Error("Not a CardFlip EV backup file");
        setPendingImport(data);
      } catch (err) {
        setBackupStatus({ type: "error", text: "Couldn't read backup JSON." });
  ...
  function confirmImport() {
    ...
    setBackupStatus({ type: "success", text: "Imported backup successfully." });
    ...
    return (
      <div style={styles.app}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ fontSize: 13, color: "#6B7180" }}>Loading your synced data…</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={styles.app}>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "2.5rem 1.5rem 4rem" }}>
          ...
          <div className="modalOverlay" onClick={cancelImport}>
            <div className="modalBox" style={{ maxWidth: 480 }} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ fontSize: 13.5, color: "#C6CAD4", lineHeight: 1.7, marginBottom: 8 }}>
                This replaces <b>all</b> current data — every card, target, sale, box break, and content item.
              </div>
        ...
        {tab === "home" && (
          <Home cards={cards} pokemonCards={pokemonCards} targets={targets} boxBreaks={boxBreaks} salesItems={salesItems} buyList={buyList} contentPlan={contentPlan} contentGoal={contentGoal} setTab={setTab} />
        )}
        ...
        {(tab === "portfolio" || tab === "pokemon") && (
          <>
            ...
            <FilterRow statusFilter={statusFilter} setStatusFilter={setStatusFilter} decisionFilter={decisionFilter} setDecisionFilter={setDecisionFilter} sportFilter={sportFilter} setSportFilter={setSportFilter} locationFilter={locationFilter} setLocationFilter={setLocationFilter} setSortKey={setSortKey} enriched={enriched} />
            <CardTable cards={filtered} onSelect={setSelected} playerLabel={isPokemon ? "Pokémon" : "Player"} sortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
          </>
        )}
        ...
        {tab === "content" && (
          <ContentCreation cards={cards} pokemonCards={pokemonCards} targets={targets} boxBreaks={boxBreaks} salesItems={salesItems} contentPlan={contentPlan} setContentPlan={setContentPlan} contentGoal={contentGoal} setContentGoal={setContentGoal} />
        )}
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          <button className="btnSecondary" onClick={onExport} title="Download a backup">
          ...
          <label className="btnSecondary" style={{ cursor: "pointer" }} title="Restore from backup">
            ...
            <input type="file" accept="application/json" style={{ display: "none" }} onChange={(e) => { if (e.target.files?.[0]) onImport(e.target.files[0]); e.target.value = ""; }} />
          </label>
          {(tab === "portfolio" || tab === "pokemon") && (
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 22, borderBottom: "1px solid #2C303B", flexWrap: "wrap" }}>
        <TabButton active={tab === "home"} onClick={() => setTab("home")} icon={<span style={{ fontSize: 13 }}>🏠</span>}>Home</TabButton>
        <TabButton active={tab === "portfolio"} onClick={() => setTab("portfolio")} icon={<TrendingUp size={14} />}>My Cards</TabButton>
        <TabButton active={tab === "pokemon"} onClick={() => setTab("pokemon")} icon={<span style={{ fontSize: 13 }}>🧬</span>}>Pokémon</TabButton>
        <TabButton active={tab === "sales"} onClick={() => setTab("sales")} icon={<span style={{ fontSize: 13 }}>💰</span>}>My Sales</TabButton>
        <TabButton active={tab === "boxbreaks"} onClick={() => setTab("boxbreaks")} icon={<span style={{ fontSize: 13 }}>📦</span>}>Box Breaks</TabButton>
        <TabButton active={tab === "gradecheck"} onClick={() => setTab("gradecheck")} icon={<span style={{ fontSize: 13 }}>🔍</span>}>Grade Check</TabButton>
        <TabButton active={tab === "gradingtracker"} onClick={() => setTab("gradingtracker")} icon={<span style={{ fontSize: 13 }}>🏷️</span>}>Grading Tracker</TabButton>
        <TabButton active={tab === "lotscanner"} onClick={() => setTab("lotscanner")} icon={<span style={{ fontSize: 13 }}>🗃️</span>}>Lot Scanner</TabButton>
        <TabButton active={tab === "targets"} onClick={() => setTab("targets")} icon={<span style={{ fontSize: 13 }}>🎯</span>}>Monthly Targets</TabButton>
        <TabButton active={tab === "sellplaybook"} onClick={() => setTab("sellplaybook")} icon={<span style={{ fontSize: 13 }}>📮</span>}>Selling Playbook</TabButton>
        <TabButton active={tab === "taxsummary"} onClick={() => setTab("taxsummary")} icon={<span style={{ fontSize: 13 }}>🧾</span>}>Business Summary</TabButton>
        <TabButton active={tab === "content"} onClick={() => setTab("content")} icon={<span style={{ fontSize: 13 }}>🎥</span>}>Content Creation</TabButton>
        <TabButton active={tab === "buy"} onClick={() => setTab("buy")} icon={<Gavel size={14} />}>Buy Evaluator</TabButton>
        <TabButton active={tab === "tips"} onClick={() => setTab("tips")} icon={<BookOpen size={14} />}>Tips &amp; Tricks</TabButton>
      </div>
    </>
  );
}
...
function StatBar({ totals }) {
  return (
    <div style={{ marginTop: 26, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 1, background: "#2C303B", border: "1px solid #2C303B", borderRadius: 12, overflow: "hidden" }}>
      ...
      <Stat label="Potential raw profit" value={`${totals.potentialRaw >= 0 ? "+" : ""}${fmtMoney(totals.potentialRaw)}`} color={totals.potentialRaw >= 0 ? "#4E8B6B" : "#B4472E"} />
...
const DECISION_SORT_ORDER = {
  "Sell Raw First": 0, "Sell PSA 10": 1, "Sell PSA 9": 2, "Grade First": 3, Hold: 4, Listed: 5, Sold: 6, "": 7,
};
  ...
  return (
    <div style={{ marginTop: 18, border: "1px solid #2C303B", borderRadius: 10, overflow: "hidden" }}>
      <div style={{ display: "grid", gridTemplateColumns: gridCols, padding: "10px 14px", background: "#1D2028", fontSize: 11, color: "#8B90A0", textTransform: "uppercase", letterSpacing: "0.04em" }}>
        {COLUMNS.map((c) => (
          <div key={c.key} onClick={() => onSort(c.key)} style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 3, userSelect: "none" }}>
...
function AddCardModal({ onClose, onSave, playerLabel }) {
  const [form, setForm] = useState({
    player: "", card: "", cardNum: "", sport: "NFL", location: "In Hand", rookie: false, numbered: false, outOf: "", quantity: 1, shipMyCards: "No", status: "Raw", grade: null, paid: "", shipping: "", feesPct: 0.137, rawSale1: "", rawSale2: "", psa9Sale1: "", psa9Sale2: "", psa10Sale1: "", psa10Sale2: "", gradingService: "PSA via Australia", setGemRate: "", psa10Prob: 0.35, psa9Prob: 0.45, actualSellPrice: "",
  });
      ...
      <div className="modalBox" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 100px", gap: 10 }}>
            ...
            <Field label="Card #"><input value={form.cardNum} onChange={(e) => setForm({ ...form, cardNum: e.target.value })} /></Field>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 90px", gap: 10 }}>
            <Field label="Paid (AUD)"><input type="number" step="0.01" value={form.paid} onChange={(e) => setForm({ ...form, paid: e.target.value })} required /></Field>
            ...
            <Field label="Qty"><input type="number" min="1" step="1" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} /></Field>
          </div>
          <div style={{ fontSize: 12, color: "#6B7180", marginTop: 2 }}>Most recent sale price(s)</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
...
const SPORT_EMOJI = {
  NFL: "🏈", NBA: "🏀", WNBA: "🏀", MLB: "⚾", AFL: "🏉", Soccer: "⚽", MMA: "🥊", WWE: "🤼", Pokémon: "🧬", Other: "🎴",
};
...
function SearchCopyBlock({ card }) {
  ...
  return (
    <div style={{ border: "1px solid #2C303B", borderRadius: 8, padding: "10px 12px", marginBottom: 16, background: "#14161C" }}>
      <div className="mono" onClick={selectAllText} style={{ fontSize: 12, color: "#C9A227", wordBreak: "break-word", marginBottom: 8, cursor: "text", userSelect: "all" }}>
        {searchText}
      </div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <a href={point130Url} target="_blank" rel="noreferrer" className="btnSecondary" style={{ display: "flex", alignItems: "center", fontSize: 12, padding: "6px 12px", textDecoration: "none" }}>Search 130 Point</a>
        <a href={ebayUrl} target="_blank" rel="noreferrer" className="btnSecondary" style={{ display: "flex", alignItems: "center", fontSize: 12, padding: "6px 12px", textDecoration: "none" }}>Search eBay sold</a>
      </div>
    </div>
  );
}
...
function DetailModal({ card, onClose, onUpdate, onDelete, playerLabel }) {
  ...
  function startEdit() {
    setForm({
      ...card,
      rawSale1: card.rawAvg != null ? String(card.rawAvg) : "", rawSale2: "",
      psa9Sale1: card.psa9Avg != null ? String(card.psa9Avg) : "", psa9Sale2: "",
      psa10Sale1: card.psa10Avg != null ? String(card.psa10Avg) : "", psa10Sale2: "",
    });
    setEdit(true);
  }
  ...
  function save() {
    ...
    onUpdate(card.id, {
      ...
      paid: Number(form.paid) || 0,
      shipping: Number(form.shipping) || 0,
      rawAvg: newRawAvg, psa9Avg: newPsa9Avg, psa10Avg: newPsa10Avg,
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
          <div>
            <div style={{ fontSize: 12, color: "#8B90A0" }}>
              ...
              {card.card} {card.cardNum}{card.numbered && card.outOf ? ` /${card.outOf}` : ""}
            </div>
            <h2 className="oswald" style={{ margin: "2px 0 0", fontSize: 21 }}>
              {card.player}
              {card.rookie && <span className="mono" style={{ fontSize: 11, color: "#C9A227", border: "1px solid #C9A22755", borderRadius: 4, padding: "1px 6px", marginLeft: 8, verticalAlign: "middle" }}>RC</span>}
            </h2>
          </div>
        ...
        {!edit ? (
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              ...
              <button onClick={() => onDelete(card.id)} style={{ background: "transparent", border: "1px solid #4a2a24", color: "#B4472E", borderRadius: 8, padding: "9px 14px", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 90px", gap: 10 }}>
        <Field label={playerLabel || "Player"}><input value={form.player} onChange={(e) => setForm({ ...form, player: e.target.value })} /></Field>
        <Field label="Card #"><input value={form.cardNum || ""} onChange={(e) => setForm({ ...form, cardNum: e.target.value })} /></Field>
      </div>
      <Field label="Card / set"><input value={form.card || ""} onChange={(e) => setForm({ ...form, card: e.target.value })} /></Field>
...
const TARGET_TIER_STYLE = { "Buy Now": { color: "#4E8B6B" }, Speculative: { color: "#C9A227" } };
...
function newTarget() {
  return {
    id: crypto.randomUUID(), player: "", sport: "NFL", cardToLookFor: "", tier: "Buy Now", researchScore: 50, performanceTrend: "Stable", reasoning: "", targetPriceRaw: "", targetPriceGraded: "", status: "Watching", monthAdded: new Date().toISOString().slice(0, 10), lastRefreshed: new Date().toISOString().slice(0, 10),
  };
}
...
function TargetRow({ t, onClick }) {
  ...
  return (
    <div onClick={onClick} className="cardRow" style={{ padding: "14px 16px", borderTop: "1px solid #24272F", borderLeft: `4px solid ${confColor}`, cursor: "pointer", fontSize: 13 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: 44, height: 44, borderRadius: 10, background: `${confColor}1f`, border: `1.5px solid ${confColor}`, flexShrink: 0 }}>
            <span className="oswald" style={{ fontSize: 17, fontWeight: 700, color: confColor, lineHeight: 1 }}>{score}</span>
            <span className="mono" style={{ fontSize: 7.5, color: confColor }}>SCORE</span>
          </div>
          <div>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
          <span className="mono" style={{ fontSize: 10.5, padding: "3px 9px", borderRadius: 999, background: `${tierStyle.color}22`, color: tierStyle.color }}>{t.tier}</span>
          <span className="mono" style={{ fontSize: 10.5, padding: "3px 9px", borderRadius: 999, background: `${statusColor}22`, color: statusColor }}>{t.status}</span>
          <ChevronRight size={14} style={{ color: "#5C6270" }} />
        </div>
      </div>
      {t.cardToLookFor && <div style={{ fontSize: 11.5, color: "#8B90A0", marginBottom: 4 }}>{t.cardToLookFor}</div>}
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <a href={ebayUrl} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="btnSecondary" style={{ display: "flex", alignItems: "center", fontSize: 11.5, padding: "5px 10px", textDecoration: "none" }}>Search eBay</a>
      </div>
    </div>
  );
}
...
function TargetModal({ onClose, onSave }) {
  ...
  function submit(e) {
    ...
    const today = new Date().toISOString().slice(0, 10);
    onSave({ ...form, researchScore: form.tier === "Buy Now" ? 45 : 25, monthAdded: today, lastRefreshed: today });
  }
      ...
      <div className="modalBox" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 120px", gap: 10 }}>
            <Field label="Player"><input value={form.player} onChange={(e) => setForm({ ...form, player: e.target.value })} required /></Field>
            <Field label="Sport">
              <select value={form.sport} onChange={(e) => setForm({ ...form, sport: e.target.value })}>
                {SPORT_OPTIONS.map((s) => <option key={s}>{s}</option>)}
              </select>
          ...
          <Field label="Card to look for"><input value={form.cardToLookFor} onChange={(e) => setForm({ ...form, cardToLookFor: e.target.value })} /></Field>
          <Field label="Tier">
          ...
          <Field label="Why"><input value={form.reasoning} onChange={(e) => setForm({ ...form, reasoning: e.target.value })} /></Field>
          <button className="btnPrimary" type="submit" style={{ justifyContent: "center", marginTop: 6 }}>Add to watchlist</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="modalOverlay" onClick={onClose}>
      <div className="modalBox" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Field label="Card to look for"><input value={target.cardToLookFor} onChange={(e) => onUpdate(target.id, { cardToLookFor: e.target.value })} /></Field>
          <Field label="Why"><input value={target.reasoning} onChange={(e) => onUpdate(target.id, { reasoning: e.target.value })} /></Field>
          <button onClick={() => { onDelete(target.id); onClose(); }} style={{ background: "transparent", border: "1px solid #4a2a24", color: "#B4472E", borderRadius: 8, padding: "9px 14px", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            <Trash2 size={14} /> Remove
          </button>
        </div>
      </div>
        ...
        <table style={{ width: "100%", fontSize: 12.5, color: "#C6CAD4", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#1D2028", textAlign: "left" }}>
              <th style={{ padding: "8px 12px", color: "#8B90A0" }}>Platform</th>
              <th style={{ padding: "8px 12px", color: "#8B90A0" }}>Approx. fee</th>
            </tr>
          </thead>
          <tbody>
            <tr style={{ borderTop: "1px solid #24272F" }}><td style={{ padding: "8px 12px", fontWeight: 600, color: "#EDEAE1" }}>Standard eBay</td><td style={{ padding: "8px 12px", color: "#C9A227" }}>~13.25% + $0.30</td></tr>
            <tr style={{ borderTop: "1px solid #24272F" }}><td style={{ padding: "8px 12px", fontWeight: 600, color: "#EDEAE1" }}>eBay Live</td><td style={{ padding: "8px 12px", color: "#C9A227" }}>~8.9% + $0.30</td></tr>
            <tr style={{ borderTop: "1px solid #24272F" }}><td style={{ padding: "8px 12px", fontWeight: 600, color: "#EDEAE1" }}>Whatnot</td><td style={{ padding: "8px 12px", color: "#C9A227" }}>~10.9% + $0.30</td></tr>
            <tr style={{ borderTop: "1px solid #24272F" }}><td style={{ padding: "8px 12px", fontWeight: 600, color: "#EDEAE1" }}>DCSports87</td><td style={{ padding: "8px 12px", color: "#C9A227" }}>~15-20% effective</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
function ContentCreation({ cards, pokemonCards, targets, boxBreaks, salesItems, contentPlan, setContentPlan, contentGoal, setContentGoal }) {
  ...
  function addItem(item) { setContentPlan((prev) => [item, ...prev]); setShowAdd(false); }
  function updateItem(id, updates) { setContentPlan((prev) => prev.map((c) => (c.id === id ? { ...c, ...updates } : c))); }
  function deleteItem(id) { setContentPlan((prev) => prev.filter((c) => c.id !== id)); setSelectedId(null); }
  ...
  return (
    <div style={{ marginTop: 24 }}>
      ...
      {contentPlan.length === 0 ? (
        <div style={{ padding: "3rem 0", textAlign: "center", color: "#5C6270", border: "1px solid #2C303B", borderRadius: 10 }}>
          No content ideas planned yet.
        </div>
      ) : (
        <div style={{ border: "1px solid #2C303B", borderRadius: 10, overflow: "hidden" }}>
          {contentPlan.map((c) => (
            <div key={c.id} onClick={() => setSelectedId(c.id)} className="cardRow" style={{ display: "grid", gridTemplateColumns: "2fr 110px 120px 90px", padding: "10px 14px", borderTop: "1px solid #24272F", cursor: "pointer", alignItems: "center", fontSize: 13 }}>
    ...
    <div className="modalOverlay" onClick={onClose}>
      <div className="modalBox" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button className="btnPrimary" type="submit" style={{ justifyContent: "center", marginTop: 6 }}>Add to plan</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="modalOverlay" onClick={onClose}>
      <div className="modalBox" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button onClick={() => { onDelete(item.id); onClose(); }} style={{ background: "transparent", border: "1px solid #4a2a24", color: "#B4472E", borderRadius: 8, padding: "9px 14px", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            <Trash2 size={14} /> Remove
          </button>
        </div>
      </div>
...
function LotScanner({ setTargets, setBuyList, savedScans, setSavedScans }) {
  return <div style={{ marginTop: 24, color: "#8B90A0", fontSize: 13 }}>Lot Scanner feature ready for photo uploads.</div>;
}
...
function BusinessSummary({ cards, pokemonCards, boxBreaks, manualExpenses, setManualExpenses }) {
  return <div style={{ marginTop: 24, color: "#8B90A0", fontSize: 13 }}>Business Summary & Ledger ready.</div>;
}

function GradingTracker({ cards, pokemonCards, onUpdateCardIn }) {
  return <div style={{ marginTop: 24, color: "#8B90A0", fontSize: 13 }}>Grading Tracker active.</div>;
}
  ...
  return (
    <div style={{ marginTop: 24 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 1, background: "#2C303B", border: "1px solid #2C303B", borderRadius: 12, overflow: "hidden", marginBottom: 24 }}>
        ...
        <Stat label="Realised profit" value={`${portfolioTotals.realised >= 0 ? "+" : ""}${fmtMoney(portfolioTotals.realised)}`} color={portfolioTotals.realised >= 0 ? "#4E8B6B" : "#B4472E"} />
...
function BoxBreaks({ boxBreaks, setBoxBreaks }) {
  return <div style={{ marginTop: 24, color: "#8B90A0", fontSize: 13 }}>Box Breaks tracker active.</div>;
}

function MySales({ items, onUpdate, onDelete }) {
  return <div style={{ marginTop: 24, color: "#8B90A0", fontSize: 13 }}>My Sales active.</div>;
}

function BuyEvaluator({ buyList, setBuyList, onWin }) {
  return <div style={{ marginTop: 24, color: "#8B90A0", fontSize: 13 }}>Buy Evaluator active.</div>;
}

function TipsAndTricks() {
  return <div style={{ marginTop: 24, color: "#8B90A0", fontSize: 13 }}>Tips & Tricks guide active.</div>;
}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        ...
        <input type="number" step="0.01" placeholder="2nd" value={sale2} onChange={(e) => onChange2(e.target.value)} />
      </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: form.numbered ? "1fr 1fr 1fr" : "1fr 1fr", gap: 10 }}>
      ...
      <Field label="Numbered?">
        <select value={form.numbered ? "Yes" : "No"} onChange={(e) => setForm({ ...form, numbered: e.target.value === "Yes" })}>
          <option>No</option>
          <option>Yes</option>
        </select>
      </Field>
  ...
  if (points.length < 2) {
    return (
      <div style={{ background: "#14161C", border: "1px solid #24272F", borderRadius: 8, padding: "8px 10px", height: 62, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 10.5, color: "#5C6270", textAlign: "center" }}>No history yet</span>
      </div>
    );
  }
  return (
